...
function makeAutoSku(name: string, rowNumber: number): string {
  const base = normalizeText(name)
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "")
    .slice(0, 24) || "PRODUCT";
  const suffix = `${Date.now().toString(36)}${rowNumber.toString(36)}`.toUpperCase();
  return `AUTO-${base.toUpperCase()}-${suffix}`;
}
...
export function parseInventoryImportFile(content: string): RawImportRow[] {
  ...
  for (let i = 1; i < lines.length; i += 1) {
    ...
    rows.push({
      rowNumber: i + 1,
      sourceRowIndex: i + 1,
      sku: get(cells, ["sku", "codigo", "código"]),
...
function parseRowsFromMatrix(matrix: string[][]): ParseResult {
  ...
  for (let rowIndex = 1; rowIndex < matrix.length; rowIndex += 1) {
    ...
    rows.push({
      ...
      sku: readCell(cells, mapping.sku),
...
export async function parseInventoryImportPayload(input: Pick<PreviewInput, "fileContent" | "fileBase64" | "fileName">): Promise<ParseResult> {
  if (input.fileBase64) {
    try {
      const pythonScript = `
import base64, io, json, sys, zipfile, xml.etree.ElementTree as ET
...
for row in root.findall(".//a:sheetData/a:row", ns):
    ...
    if max_col == 0:
        rows.append([])
        continue
    arr = [cells.get(i, "") for i in range(1, max_col + 1)]
    rows.append(arr)
print(json.dumps(rows))
`;
      const json = execFileSync("python3", ["-c", pythonScript], {
        input: input.fileBase64,
        encoding: "utf8",
        maxBuffer: 1024 * 1024 * 20,
      });
      const matrix = JSON.parse(json) as string[][];
...
export async function previewInventoryImport(input: PreviewInput) {
  ...
  const allSkus = Array.from(new Set(parsedRows.map((row) => row.sku.trim().toUpperCase()).filter(Boolean)));
  ...
  for (const item of input.items) {
    ...
    try {
      ...
      if (!product) {
        ...
        const skuToCreate = cleanSku || makeAutoSku(cleanName, item.rowNumber);
