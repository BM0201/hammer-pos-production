  ...
  for (const order of discountRates) {
    ...
    if (order.discountRate > 30 || (sd > 0 && zScore(order.discountRate, m, sd) > 2.5)) {
      discrepancies.push({
        ...
        description: `Orden ${order.orderNumber} en "${order.branch.name}" por ${order.createdBy.username} tiene un descuento del ${order.discountRate.toFixed(1)}% (C$ ${Number(order.discountTotal).toFixed(2)} de C$ ${Number(order.subtotal).toFixed(2)}). Promedio de descuento: ${m.toFixed(1)}%.`,
      ...
      if (Math.abs(totalA - totalB) < 0.01 && totalA > 0) {
        discrepancies.push({
          ...
          details: {
            ...
            userA: a.createdBy.username,
            userB: b.createdBy.username,
          },
...
async function detectAnomalousReturns(
  ...
  const userReturnCount = new Map<string, { count: number; username: string; branch: string }>();
  for (const r of returnOrders) {
    const key = r.createdByUserId;
    if (!userReturnCount.has(key)) {
      userReturnCount.set(key, { count: 0, username: r.createdBy.username, branch: r.branch.name });
    }
  ...
  for (const [userId, data] of userReturnCount) {
    if (data.count >= 3) {
      discrepancies.push({
        ...
        category: "discrepancy",
        title: `Alto número de devoluciones por usuario: ${data.username}`,
        description: `${data.username} (${data.branch}) procesó ${data.count} devoluciones en los últimos ${DAYS_WINDOW} días. Verificar procedimiento.`,
