...
export async function POST(request: Request) {
  try {
    ...
    if (payload?.mode === "preview") {
      ...
      if (!parsed.success) {
        return NextResponse.json({ message: "Payload inválido para preview.", issues: parsed.error.issues }, { status: 400 });
      }
    ...
    if (payload?.mode === "execute") {
      ...
      if (!parsed.success) {
        return NextResponse.json({ message: "Payload inválido para ejecución.", issues: parsed.error.issues }, { status: 400 });
      }
    ...
    return NextResponse.json({ message: "mode debe ser preview o execute." }, { status: 400 });
  } catch (error) {
