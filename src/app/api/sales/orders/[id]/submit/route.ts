...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const payload = await request.json().catch(() => ({})) as { requiresTransport?: unknown; transportAmount?: unknown };
    const requiresTransport = typeof payload.requiresTransport === "boolean"
      ? payload.requiresTransport
      : undefined;
    const transportAmount = typeof payload.transportAmount === "number" && payload.transportAmount >= 0
