 ...
 *
 * Fix: Fusiones ahora también calcula effectiveCost (resolveCatalogDisplayCost,
 * el mismo motor de "costo de red" que ya usa el catálogo, con el WAC
 * agregado de todas las sucursales) y calcula el margen con ESE número,
 * no con globalCost — y avisa en la fila cuando difieren, para que
 * editar el costo global de ahí no se sienta como que "no hizo nada".
 *
...
test("backend: el costo efectivo de cada miembro usa resolveCatalogDisplayCost (el mismo motor de costo de red del catálogo), no una quinta cascada", () => {
  const service = readApi("hammer-api/src/modules/catalog/stock-group-crud.ts");
  assert.ok(service.includes('import { resolveCatalogDisplayCost } from "@/modules/catalog-inventory/service"'));
  assert.ok(service.includes("effectiveCostRaw"), "debe calcular el costo efectivo por miembro, separado de globalCost");
});
