...
export function LoginForm() {
  ...
  async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        // Contrato estándar: { ok: false, error: { code, message } }
        const apiErr = json as ApiResponse<unknown>;
        if (!apiErr.ok && "error" in apiErr) {
          const msg = typeof apiErr.error === "object" && apiErr.error && "message" in apiErr.error
            ? (apiErr.error as { message: string }).message
            : "Usuario o contraseña inválidos.";
          setError(msg);
        } else {
          setError(response.status === 401 ? "Usuario o contraseña inválidos." : "No se pudo iniciar sesión.");
        }
        return;
      }

      const data = unwrapApiData(json as ApiResponse<{ redirectTo: string; mustChangePassword: boolean }>);
      if (!data.redirectTo) {
