...
const REPORTS: ReportItem[] = [
  ...
  { key: "audit", label: "Bitácora", endpoint: "/api/reports/audit" },
  { key: "inventory-critical", label: "Inventario crítico", endpoint: "/api/reports/inventory-critical" },
];
  ...
  return (
    <section className="space-y-4">
      <div>
        ...
        <p className="text-sm text-[var(--color-text-muted)]">Descarga CSV por módulo con filtros opcionales.</p>
      </div>
      ...
      <div className="grid gap-3 md:grid-cols-5">
        ...
        <label className="text-sm space-y-1">
          <span className="text-[var(--color-text-muted)]">Estado / Acción</span>
          <input className="w-full rounded-lg border border-[var(--color-border)] px-2 py-1.5" value={status} onChange={(event) => setStatus(event.target.value)} />
        </label>
        <label className="text-sm space-y-1">
