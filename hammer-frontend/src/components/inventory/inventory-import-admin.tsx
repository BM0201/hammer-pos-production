...
export function InventoryImportAdmin({ branches, categories }: { branches: Branch[]; categories: Category[] }) {
  ...
  async function handlePreview() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(raw.error?.message ?? raw.message ?? "No se pudo generar preview.");
      }
  ...
  async function handleExecute() {
    ...
    try {
      ...
      const raw = await response.json();
      if (!response.ok) {
        throw new Error(raw.error?.message ?? raw.message ?? "No se pudo ejecutar importación.");
      }

      const data = unwrapApiData(raw) as { insertados: number; actualizados: number; omitidos: number; errores: number };
      setFeedback({
        tone: "success",
