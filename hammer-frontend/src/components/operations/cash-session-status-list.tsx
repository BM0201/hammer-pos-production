                ...
                <div key={session.id} className={`rounded-xl border ${URGENCY_BORDER[urgency]} overflow-hidden`}>
                  ...
                  <div className="grid grid-cols-2 gap-0 border-t border-[var(--color-border)] divide-x divide-[var(--color-border)] sm:grid-cols-4">
                    {[
                      ...
                      { label: "Esperado",  value: session.expectedCashAmount === null ? "—" : money(session.expectedCashAmount) },
                      { label: "Contado",   value: session.countedCashAmount === null ? "—" : money(session.countedCashAmount) },
                    ].map(({ label, value }) => (
                    ...
                    <div className="flex flex-col gap-0.5 px-3 py-2">
                      ...
                      <span className={`text-xs ${session.differenceAmount === null ? "text-[var(--color-text-muted)]" : diffColor(diff)}`}>
                  ...
                  {session.status === "OPEN" && closing && (
                    <div className="flex flex-col gap-2 border-t border-[var(--color-danger-200)] bg-[color-mix(in_srgb,var(--color-danger-50)_30%,white)] px-3.5 py-2.5 sm:flex-row sm:items-center sm:justify-between">
                      ...
                      {branchId && (
                        <Link
                          href="/app/branch/cash"
                          className="flex items-center justify-center gap-1.5 rounded-lg border border-[var(--color-danger-300)] bg-[var(--color-surface)] px-3 py-2 text-xs font-semibold text-[var(--color-danger-700)] hover:bg-[var(--color-danger-50)] transition-colors sm:w-auto w-full"
                        >
                          Cerrar caja <ArrowRight style={{ width: "0.75rem", height: "0.75rem" }} />
                        </Link>
                      )}
                    </div>
                  )}
                  {session.status === "OPEN" && !closing && (
                    <div className="flex flex-col gap-2 border-t border-[var(--color-border)] bg-[var(--color-surface-muted)] px-3.5 py-2.5 sm:flex-row sm:items-center sm:justify-between">
                      ...
                      {branchId && (
                        <Link
                          href="/app/branch/cash"
                          className="flex items-center justify-center gap-1.5 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] px-3 py-2 text-xs font-semibold text-[var(--color-text-secondary)] hover:text-[var(--color-text)] hover:bg-[var(--color-surface-alt)] transition-colors sm:w-auto w-full"
                        >
