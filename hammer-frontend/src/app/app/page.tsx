...
export default function SettingsPage() {
  ...
  const loadSettings = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        const raw = json?.data ?? unwrapApiData(json);
        setSettings(Array.isArray(raw) ? raw : []);
      } else {
        setNotice(json.error?.message ?? json.message ?? "Error al cargar");
      }
    } catch {
  ...
  async function handleSave(key: string, value: string) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const json = await res.json();
        setNotice(json.error?.message ?? json.message ?? "Error al guardar");
      }
    } catch {
      setNotice("Error de conexión");
    }
