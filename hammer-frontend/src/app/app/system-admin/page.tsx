  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const branchData = unwrapApiData(await branchRes.json());
        const roleData = unwrapApiData(await roleRes.json());
        ...
        setStats({
          ...
          users: 0,
          activeDiscounts: 0,
          roleConfigs: roles.reduce((sum: number, b: { roles: unknown[] }) => sum + (b.roles?.length ?? 0), 0),
        });
      } catch { /* ignore */ }
      setLoading(false);
    }
