...
const CONFIGURABLE_ROLES = [
  ...
  { code: "WAREHOUSE", label: "Despacho / Bodega", description: "Despacho, inventario y produccion" },
];
  ...
  const loadConfigs = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setNotice(json.error?.message ?? json.message ?? "Error al cargar configuración");
      }
    } catch {
  ...
  async function toggleRole(branchId: string, roleCode: string) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const json = await res.json();
        setNotice(json.error?.message ?? json.message ?? "Error al actualizar");
      }
    } catch {
      setNotice("Error de conexión");
    }
