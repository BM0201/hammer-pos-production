  ...
  return (
    <section className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 ml-4">Gestiona las recetas de fabricación</p>
      </div>
      ...
      {/* Results count */}
      <p className="text-xs text-gray-500">{filtered.length} receta{filtered.length !== 1 ? "s" : ""} encontrada{filtered.length !== 1 ? "s" : ""}</p>
      ...
      <div className="bg-white rounded-xl border shadow-sm">
        ...
        {!loading && !error && filtered.length === 0 && (
          <p className="px-4 py-8 text-center text-sm text-gray-400">No se encontraron recetas.</p>
        )}
                ...
                {filtered.map((r) => (
                  <tr key={r.id} className="hover:bg-gray-50">
                    <td className="px-4 py-2.5">
                      ...
                      <Link href={`/app/master/production/recipes/${r.id}` as any} className="text-indigo-600 hover:underline font-mono text-xs">
                        {r.code}
                      </Link>
                    </td>
                    <td className="px-4 py-2.5 font-medium text-gray-900">{r.name}</td>
                    <td className="px-4 py-2.5 text-gray-600">{r.finishedProduct.name}</td>
                    <td className="px-4 py-2.5 text-right text-gray-700">{r.expectedQuantity.toLocaleString()} {r.expectedUnit}</td>
                    <td className="px-4 py-2.5 text-center text-gray-600">{r.inputs.length}</td>
                    <td className="px-4 py-2.5 text-center text-gray-600">{r._count.batches}</td>
                    <td className="px-4 py-2.5 text-center">
                      ...
                      <div className="flex items-center justify-center gap-0.5">
                        {[1, 2, 3, 4, 5].map((i) => (
                          <div
                            key={i}
                            className={`w-2 h-2 rounded-full ${i <= estimateCostIndicator(r) ? "bg-indigo-500" : "bg-gray-200"}`}
                          />
                        ))}
                      </div>
                      <span className="text-[10px] text-gray-400">{r.inputs.length} insumos</span>
                    </td>
                    <td className="px-4 py-2.5 text-center">
                      <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium ${r.isActive ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}>
      ...
      {/* Back link */}
      <div className="mt-4">
        <Link href="/app/master/production" className="text-sm text-indigo-600 hover:underline">← Volver al Dashboard</Link>
      </div>
    </section>
  );
}
