...
function getCostDeviation(batch: Batch): { value: number; label: string; color: string } | null {
  ...
  if (absVal < 5) return { value: deviation, label: `${deviation > 0 ? "+" : ""}${deviation.toFixed(1)}%`, color: "text-green-600" };
  if (absVal < 15) return { value: deviation, label: `${deviation > 0 ? "+" : ""}${deviation.toFixed(1)}%`, color: "text-yellow-600" };
  return { value: deviation, label: `${deviation > 0 ? "+" : ""}${deviation.toFixed(1)}%`, color: "text-red-600" };
}
  ...
  return (
    <section className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 ml-4">Listado y gestión de lotes de producción</p>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-3 items-start md:items-center flex-wrap">
        <input
          ...
          className="flex-1 min-w-[200px] px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
        />
        <select
          value={statusFilter}
      ...
      <p className="text-xs text-gray-500">{filtered.length} lote{filtered.length !== 1 ? "s" : ""}</p>
      ...
      <div className="bg-white rounded-xl border shadow-sm">
        ...
        {!loading && !error && filtered.length === 0 && (
          <p className="px-4 py-8 text-center text-sm text-gray-400">No se encontraron lotes.</p>
        )}
                ...
                {filtered.map((b) => {
                  ...
                  return (
                    <tr key={b.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-2.5 text-right text-gray-700">{b.totalCost != null ? `C$${b.totalCost.toFixed(2)}` : "—"}</td>
                      ...
                      <td className="px-4 py-2.5 text-gray-500 text-xs">{new Date(b.createdAt).toLocaleDateString("es")}</td>
                    </tr>
                  );
                })}
              </tbody>
