...
const CD = {
  ...
  accentHi:  "#E04010",
  accentLo:  "#A82B08",
  accentDim: "rgba(212,56,13,0.18)",
  success:          "#34D399",   successText:    "#34D399",
  info:             "#22D3EE",   infoText:       "#22D3EE",
  warning:          "#FBBF24",   warningText:    "#FBBF24",
  inactive:         "#847F76",   inactiveText:   "#847F76",
  brandAccent:      "#FB923C",   brandText:      "#FB923C",
  catInventario:    "#4ADE80",   catText:        "#4ADE80",
  roleAdmin:        "#A78BFA",   roleAdminText:  "#A78BFA",
  ink2OnBase:       "#D6D2CA",   ink3OnBase:       "#A8A39A",
  ink2OnSurface:    "#D6D2CA",   ink3OnSurface:    "#A8A39A",
  ink2OnSurfaceHi:  "#D6D2CA",   ink3OnSurfaceHi:  "#A8A39A",
  green:     "#34D399",
} as const;
...
export default function LoginPage() {
  ...
  /* Theme — reads saved preference from HTML data-theme, no forced override */
  ...
  /* Style helpers — close over current palette */
  ...
  function inputS(hasErr: boolean, extraPad?: string): CSSProperties {
    return {
      width: "100%",
      background: p.surface,
      border: `1px solid ${hasErr ? p.accent : p.border}`,
      borderRadius: "5px",
      color: p.ink,
      fontFamily: "'DM Mono', monospace",
      fontSize: "14px",
      ...
      outline: "none",
      boxShadow: hasErr ? `0 0 0 3px ${p.accentDim}` : "none",
    ...
    apiFetch("/api/auth/session")
      ...
      .then((raw) => {
        ...
        if (data?.authenticated && data.user) {
          router.replace(resolveRoleHome(data.user.roleCode, data.user.globalRoles ?? []));
        }
      })
      .catch(() => {})
  ...
  function playTransition(userName: string, onDone: () => void) {
    ...
    setTimeout(() => {
      ...
      /* Use current theme bg color to avoid flash between login and splash */
      const loginBg = document.documentElement.dataset.theme === "dark" ? "#1A1917" : "#EDECEA";
      document.documentElement.style.backgroundColor = loginBg;
  ...
  if (checking) {
    return (
      <div style={{ minHeight: "100dvh", background: p.base, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{
          width: "1.25rem", height: "1.25rem", borderRadius: "50%",
          border: `2px solid ${p.border}`, borderTopColor: p.accent,
  ...
  const submitBtn = (label: string, loadingLabel: string) => (
    <button
      ...
      style={{
        ...
        marginTop: "1.25rem",
        padding: "11px 20px",
        background: submitting ? p.accentLo : btnHov ? p.accentHi : p.accent,
  ...
  const errBox = (msg: string) => (
    <div style={{
      ...
      borderRadius: "5px", fontSize: "12px", color: p.accentLo,
    }}>
  ...
  /* ── Texture values (dark-adaptive) ─────────────────────────────── */
  const dotTex = isDark
    ? "radial-gradient(circle, rgba(168,163,154,0.14) 1px, transparent 1px)"
    : "radial-gradient(circle, #C5C3BE 1px, transparent 1px)";
  const dotOp  = isDark ? 1 : 0.5;
  const gridOp = isDark ? 0.10 : 0.18;
    ...
    <>
      ...
      >
        ...
        <main className="hm-mn" style={{ gridColumn: 2, gridRow: 2, position: "relative", zIndex: 1 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "2rem" }}>
            <svg width="40" height="40" viewBox="0 0 40 40" fill="none" aria-hidden>
              <line x1="20" y1="7"  x2="7"  y2="30" stroke={p.border} strokeWidth="1.5"/>
              <line x1="20" y1="7"  x2="33" y2="30" stroke={p.border} strokeWidth="1.5"/>
              <line x1="7"  y1="30" x2="33" y2="30" stroke={p.border} strokeWidth="1.5"/>
              <circle cx="20" cy="7"  r="4.5" fill={p.ink}/>
              <circle cx="7"  cy="30" r="3.5" fill={p.ink3OnBase}/>
              <circle cx="33" cy="30" r="3.5" fill={p.ink3OnBase}/>
              <circle cx="20" cy="7"  r="2"   fill={p.accent}/>
            </svg>
            <div style={{ display: "flex", flexDirection: "column", gap: "1px", lineHeight: 1 }}>
              <span style={{ fontFamily: "'DM Mono', monospace", fontWeight: 500, fontSize: "26px", letterSpacing: "0.12em", color: p.ink }}>
                HAMMER
              </span>
              <span style={{ fontSize: "10px", color: p.ink3, letterSpacing: "0.04em", lineHeight: 1.4 }}>
          ...
          <div style={{
            ...
            overflow: "hidden",
            boxShadow: isDark
              ? "0 1px 3px rgba(0,0,0,0.45), 0 4px 16px rgba(0,0,0,0.30)"
              : "0 1px 3px rgba(0,0,0,0.06), 0 4px 16px rgba(0,0,0,0.04)",
          }}>
            {/* Card header */}
            <div style={{
              display: "flex", alignItems: "center", justifyContent: "space-between",
              padding: "10px 16px", background: p.surface, borderBottom: `1px solid ${p.border}`,
            }}>
              <div style={{ display: "flex", gap: "5px", alignItems: "center" }}>
                <div style={{ width: "8px", height: "8px", borderRadius: "50%", background: p.accent }} />
                <div style={{ width: "8px", height: "8px", borderRadius: "50%", background: p.borderMid }} />
                <div style={{ width: "8px", height: "8px", borderRadius: "50%", background: p.borderMid }} />
              </div>
              <span style={{ fontFamily: "'DM Mono', monospace", fontSize: "11px", color: p.ink3, letterSpacing: "0.06em" }}>
                ACCESO AL SISTEMA
              </span>
              <span style={{
                fontFamily: "'DM Mono', monospace", fontSize: "10px", color: "#fff",
                background: p.accent, letterSpacing: "0.08em", padding: "2px 7px", borderRadius: "3px", fontWeight: 500,
              }}>
                BETA
              </span>
            </div>
            ...
            <div style={{ padding: "1.75rem 1.75rem 1.5rem" }}>
              ...
              {step === "credentials" ? (
                <>
                  <h1 style={{ fontSize: "17px", fontWeight: 600, color: p.ink, letterSpacing: "-0.02em", marginBottom: "3px" }}>
                    Iniciar sesión
                  </h1>
                  <p style={{ fontSize: "13px", color: p.ink2, marginBottom: "1.5rem" }}>
                  ...
                  <form onSubmit={handleCredentials} noValidate>
                    ...
                    <div style={{ marginBottom: ".875rem" }}>
                      ...
                      <div style={{ position: "relative" }}>
                        <svg aria-hidden style={{ position: "absolute", left: "11px", top: "50%", transform: "translateY(-50%)", width: "14px", height: "14px", color: p.ink3, pointerEvents: "none" }} viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5">
                      ...
                      {errUser && (
                        <span style={{ display: "block", marginTop: "4px", fontSize: "11px", color: p.accent, fontWeight: 500, fontFamily: "'DM Mono', monospace" }}>
                          {"// campo requerido"}
                        </span>
                      )}
                    </div>
                      ...
                      <div style={{ position: "relative" }}>
                        ...
                        <button
                          ...
                          style={{
                            ...
                            background: "none", border: "none", cursor: "pointer", color: p.ink3,
                ...
                <form onSubmit={handleMfa}>
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "8px", paddingBottom: "1.25rem" }}>
                    <div style={{
                      ...
                      background: p.accentDim, display: "flex", alignItems: "center", justifyContent: "center",
                    }}>
                      <svg width="22" height="22" viewBox="0 0 16 16" fill="none" stroke={p.accent} strokeWidth="1.5" aria-hidden>
                        <rect x="3" y="7" width="10" height="8" rx="1.5"/>
                        <path d="M5.5 7V5a2.5 2.5 0 0 1 5 0v2" strokeLinecap="round"/>
                        <circle cx="8" cy="11" r="1" fill={p.accent} stroke="none"/>
                      </svg>
                    </div>
                    <h2 style={{ fontSize: "16px", fontWeight: 600, color: p.ink, letterSpacing: "-0.02em" }}>
                      Verificación en dos pasos
                    </h2>
                    <p style={{ fontSize: "12px", color: p.ink2, textAlign: "center", maxWidth: "260px", lineHeight: 1.5 }}>
                  ...
                  <div style={{ marginBottom: "1rem" }}>
                    ...
                    <input
                      ...
                      style={{
                        ...
                        borderRadius: "5px", color: p.ink, fontFamily: "'DM Mono', monospace",
                  ...
                  <button
                    ...
                    style={{
                      ...
                      cursor: "pointer", fontSize: "12px", color: p.ink3,
                      fontFamily: "'Inter', system-ui, sans-serif",
                    }}
                  >
            ...
            {/* Card footer */}
            <div style={{ padding: "10px 1.75rem", background: p.surface, borderTop: `1px solid ${p.border}` }}>
              <p style={{ fontSize: "12px", color: p.ink3, lineHeight: 1.5 }}>
        ...
        <div className="hm-cr" style={{ position: "fixed", bottom: "1.5rem", left: "1.75rem", zIndex: 1, display: "flex", flexDirection: "column", gap: "4px" }}>
          <span style={{ fontFamily: "'DM Mono', monospace", fontSize: "10px", color: p.ink3OnBase, letterSpacing: "0.05em", textTransform: "uppercase" }}>
            Sucursal
          </span>
          <div style={{ height: "16px", overflow: "hidden" }}>
            <span style={{
              fontFamily: "'DM Mono', monospace", fontSize: "11px", color: p.ink2OnBase,
          ...
          <span style={{ fontFamily: "'DM Mono', monospace", fontSize: "10px", color: p.ink3OnBase, letterSpacing: "0.05em", textTransform: "uppercase", marginTop: "6px" }}>
            Último acceso
          </span>
          <span style={{ fontFamily: "'DM Mono', monospace", fontSize: "11px", color: p.ink2OnBase }}>
            {lastAcceso}
          </span>
        </div>
        ...
        <div className="hm-cr" style={{ position: "fixed", bottom: "1.5rem", right: "1.75rem", zIndex: 1, display: "flex", flexDirection: "column", alignItems: "flex-end", gap: "4px" }}>
          <span style={{ display: "inline-flex", alignItems: "center", gap: "5px", fontFamily: "'DM Mono', monospace", fontSize: "10px", color: p.success, letterSpacing: "0.04em" }}>
            <span style={{ width: "6px", height: "6px", borderRadius: "50%", background: p.success, animation: "hmPulse 2.5s ease-in-out infinite", flexShrink: 0 }} />
            SISTEMA OPERATIVO
          </span>
          <span style={{ fontFamily: "'DM Mono', monospace", fontSize: "11px", color: p.ink2OnBase, marginTop: "4px" }}>
            © 2026 Hammer
          </span>
        </div>
      </div>
      ...
      >
        <div aria-hidden style={{
          position: "absolute", inset: 0, pointerEvents: "none",
          backgroundImage: dotTex,
          backgroundSize: "20px 20px", opacity: isDark ? 0.8 : 0.4,
        }} />
        <div aria-hidden style={{
          position: "absolute", inset: 0, pointerEvents: "none",
          backgroundImage: `linear-gradient(${p.border} 1px, transparent 1px), linear-gradient(90deg, ${p.border} 1px, transparent 1px)`,
          backgroundSize: "80px 80px", opacity: isDark ? 0.08 : 0.14,
        }} />
        ...
        >
          <line x1="36" y1="12" x2="66" y2="64" stroke={p.border} strokeWidth="1.5"/>
          <line x1="66" y1="64" x2="6"  y2="64" stroke={p.border} strokeWidth="1.5"/>
          <line x1="6"  y1="64" x2="36" y2="12" stroke={p.border} strokeWidth="1.5"/>
          <circle cx="36" cy="12" r="6" fill={p.ink}/>
          <circle cx="66" cy="64" r="5" fill={p.ink3OnBase}/>
          <circle cx="6"  cy="64" r="5" fill={p.ink3OnBase}/>
          <circle
            ref={dotRef}
            id="hm-red-dot"
            cx="36" cy="12" r="3"
        ...
        <div style={{
          ...
          <div
            ...
            style={{
              height: "100%",
              width: "0%",
              background: `linear-gradient(90deg, ${p.accentLo}, ${p.accent})`,
