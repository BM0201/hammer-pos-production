...
export async function autoCloseExpiredCashSessions(input: {
  ...
  // Fast-skip: use a dummy branch to check whether auto-close could possibly
  // fire right now. If the deadline hasn't passed for today's schedule, no
  // session can be eligible — skip the expensive findMany entirely.
  // getCashAutoCloseDeadline is a pure function (no I/O).
  if (!input.branchId) {
    const quickCheck = getCashAutoCloseDeadline({ id: "_check" }, now, config);
    if (!quickCheck.enabled || !quickCheck.expired) {
      return {
        scanned: 0,
        autoClosed: 0,
        wouldAutoClose: 0,
  ...
  const openSessions = await prisma.cashSession.findMany({
    where: {
      ...
      ...(input.branchId ? { physicalCashBox: { branchId: input.branchId } } : {}),
    },
    include: {
