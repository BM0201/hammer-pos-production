...
import { businessDateFromNow, getOperationalWindowForNow, OPERATIONAL_TIMEZONE } from "@/modules/operations/service";
import {
...
export async function getCommandCenterSnapshot() {
  ...
  // businessDate is stored at midnight UTC (00:00). The operationalWindow.start is
  // businessDate + 6h, so a gte/lt range filter on businessDate would NEVER match
  // the stored value. Use an exact equality match instead.
  ...
  const [
    ...
    // 5. Today's operational day per branch — exact match on businessDate (midnight UTC).
    ...
    // 9. Real-time commercial sales per branch (batch: ~9 queries instead of N×10).
    getAllBranchesSalesRealtimeSummary(),
    // 10. Cash sessions tied to today's operational day — exact businessDate match.
    prisma.cashSession.findMany({
      where: { operationalDay: { businessDate: todayBusinessDate } },
      select: {
        id: true,
        status: true,
        openingAmount: true,
    ...
    // 12. Cash movements registered against today's sessions — exact businessDate match.
    prisma.cashMovement.findMany({
      where: { cashSession: { operationalDay: { businessDate: todayBusinessDate } } },
      select: {
        type: true,
        amount: true,
  ...
  const [
    ...
    // Día Operativo 360: la cola única es AWAITING_REVIEW + reviewStatus
    // PENDING — reemplaza a los dos contadores viejos (CLOSED-sin-aprobar y
    // PENDING_CLOSE), que ahora son el mismo estado.
    prisma.operationalDay.count({ where: { lifecycle: "AWAITING_REVIEW", reviewStatus: "PENDING" } }),
    prisma.operationalDay.count({ where: { lifecycle: "ACTIVE", businessDate: { lt: todayBusinessDate } } }),
