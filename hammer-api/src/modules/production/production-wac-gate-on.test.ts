...
/**
 * docs/WAC-DESACTIVADO.md — con el flag PRENDIDO, getInputWacTx y
 * getSaleStockAndCost deben seguir exactamente igual que siempre: WAC
 * directo del balance, sin el fallback a averageCost/globalCost/
 * lastPurchaseCost. Ver production-wac-gate-off.test.ts para por qué esto
 * vive en su propio archivo (cache de isWacDrivesCostChainEnabled a nivel
 * de módulo, compartido por proceso — node --test aísla por archivo).
 */
...
test("getSaleStockAndCost (motor de recomendación) con el flag prendido: sigue usando WAC directo del balance, nunca averageCost", async () => {
  const db = createFakeDb({ weightedAverageCost: new Prisma.Decimal(470), averageCost: new Prisma.Decimal(18) });
  const { unitCost } = await getSaleStockAndCost("branch-1", "prod-cemento", db);
  assert.equal(unitCost, 470, "con el flag prendido el comportamiento no cambió: WAC tal cual, sin fallback");
});
