 ...
 * effective-pricing.ts (precio, batch) y resolveCatalogDisplayCost (costo,
 * batch) — el mismo motor que el POS y el catálogo, no una tercera
 * resolución. `db` es inyectable en TODA la cadena (getCurrentPrices,
 * resolveCatalogDisplayCostBatch, resolvePolicyForProductBatch,
 * getEffectiveProductPricingBatch) — acá se le da un fake en memoria que
 * filtra arrays fijos, sin base de datos real.
 */
...
test("Test 7 — el effectiveCost del derivado de fusión (arena) coincide con resolveCatalogDisplayCostBatch para el mismo producto y sucursal", async () => {
  const db = makeFakeDb(baseFixtures());
  const result = await getCurrentPrices({ branchId: BRANCH }, db);
  const p4 = result.rows.find((r) => r.productId === "p4")!;

  const directCost = await resolveCatalogDisplayCostBatch(["p4"], BRANCH, db);
  assert.equal(p4.effectiveCost, directCost.get("p4"));
...
test("Test 8 — byPriceSource suma igual que total", async () => {
  ...
  assert.equal(total, 5, "p1, p2, p3, p-canonical, p4");
});
...
test("filtro priceSource: MISSING devuelve exactamente los que no se pueden vender (p3), sin tocar totals (que sigue siendo de toda la consulta)", async () => {
  ...
  assert.equal(result.totals.total, 5, "totals sigue siendo de la consulta completa, no de la página filtrada");
