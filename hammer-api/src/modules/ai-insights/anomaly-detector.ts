...
async function detectCashierAnomalies(
  ...
  for (const u of userSales) {
    ...
    if (Math.abs(zTotal) > 2.0) {
      ...
      const actorLabel = formatActor(user, userId);

      const direction = zTotal > 0 ? "superior" : "inferior";
      anomalies.push({
        id: `anom-cashier-total-${userId}`,
        category: "anomaly",
        title: `Cajero con ticket promedio ${direction}: ${actorLabel}`,
        description: `${actorLabel} tiene un ticket promedio de C$ ${avgTotal.toFixed(2)} — significativamente ${direction} al promedio de C$ ${totalMean.toFixed(2)} (${u._count.id} transacciones).`,
        severity: Math.abs(zTotal) > 2.5 ? "high" : "medium",
        entityType: "cashier",
        entityId: userId,
        entityName: actorLabel,
    ...
    if (discSd > 0) {
      ...
      if (zDisc > 2.0) {
        ...
        anomalies.push({
          ...
          category: "anomaly",
          title: `Descuentos excesivos: ${actorLabel}`,
          description: `${actorLabel} aplica un descuento promedio de C$ ${avgDisc.toFixed(2)} por transacción — significativamente mayor al promedio de C$ ${discMean.toFixed(2)}.`,
