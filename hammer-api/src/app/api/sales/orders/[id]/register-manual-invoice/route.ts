...
export async function POST(request: Request, { params }: RouteParams) {
  try {
    ...
    // Cargar branchId real del recurso para verificar acceso
    const order = await prisma.saleOrder.findUnique({
      where: { id },
      select: { branchId: true, manualInvoiceStatus: true },
    });
    if (!order) return validationFail({ id: "Orden no encontrada" });

    requireBranchCapability(session, order.branchId, CAPABILITIES.MANUAL_INVOICE_REGISTER);
