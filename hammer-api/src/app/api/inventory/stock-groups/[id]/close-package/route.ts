...
import { closeStockPackage } from "@/modules/inventory/service";
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    return ok(await closeStockPackage({
      actorUserId: session!.userId,
      branchId: body.branchId,
      stockGroupId: id,
      packageProductId: body.packageProductId ?? null,
      packagesToClose: body.packagesToClose ?? 0,
      actualUnitsConsumed: body.actualUnitsConsumed ?? null,
