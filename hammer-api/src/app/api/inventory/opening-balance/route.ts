...
import { createOpeningBalance } from "@/modules/inventory/service";
...
export async function POST(request: Request) {
  try {
    ...
    const parsed = openingBalanceSchema.safeParse(await request.json());
    ...
    if (!canPostMovement(session.roleCode, "ADJUSTMENT_IN")) {
      return fail("FORBIDDEN", "Role not allowed to post this movement type.", 403);
    }

    return ok(await createOpeningBalance({ ...parsed.data, actorUserId: session.userId }));
  } catch (error) {
