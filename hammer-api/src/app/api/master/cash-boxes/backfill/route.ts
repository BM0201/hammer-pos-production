  ...
  try {
    ...
    const branches = await prisma.branch.findMany({
      ...
      select: {
        ...
        code: true,
        name: true,
        _count: { select: { physicalCashBoxes: true } },
      },
      orderBy: { code: "asc" },
    });

    const branchesWithoutBox = branches.filter((branch) => branch._count.physicalCashBoxes === 0);
    ...
    return ok({
      ...
      message:
        createdBoxes.length > 0
          ...
          : "Todas las sucursales activas ya tienen al menos una caja física.",
    });
  } catch (error) {
