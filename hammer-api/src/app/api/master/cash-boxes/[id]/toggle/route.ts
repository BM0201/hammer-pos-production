...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Guard: cannot deactivate a box with live sessions.
    if (cashBox.isActive) {
      const liveSession = await prisma.cashSession.findFirst({
        where: {
          physicalCashBoxId: id,
          status: { in: ["OPEN", "RECONCILING", "AUTO_CLOSED_PENDING_REVIEW"] },
        },
        select: { id: true, status: true },
      });
      if (liveSession) {
        throw new Error(
          `VALIDATION_ERROR: No se puede desactivar la caja ${cashBox.code} porque tiene una sesion activa en estado ${liveSession.status}. Cierra la sesion primero.`,
        );
      }
    }
