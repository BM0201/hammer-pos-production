...
async function main() {
  ...
  console.log("\n───────────────────────────────────────────────");
  const isValid = verifyPassword(TEST_PASSWORD, user.passwordHash);
  console.log(`🔑 Contraseña '${TEST_PASSWORD}': ${isValid ? "✅ VÁLIDA" : "❌ INVÁLIDA"}`);

  if (!isValid) {
    console.log("\n💡 Solución: Resetear contraseña del master:");
    console.log("   DATABASE_URL=... RESET_MASTER_PASSWORD=true npx tsx prisma/seed.ts\n");
  }
